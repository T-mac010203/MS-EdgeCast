...
class ConditionalEmbedding(nn.Module):
    def __init__(self, num_labels, dim):
        ...
        self.condEmbedding = nn.Sequential(
            nn.Embedding(num_embeddings=num_labels + 1, embedding_dim=dim, padding_idx=0),
            nn.Linear(dim, dim*4),
            nn.GELU(),
            nn.Linear(dim*4, dim),
        )
...
class Upsample3D(nn.Module):
    def __init__(self, channels):
        ...
        self.conv = nn.ConvTranspose3d(channels, channels, kernel_size=(1,4,4), stride=(1,2,2), padding=(0,1,1))

    def forward(self, x):
        # x = nn.functional.interpolate(x, scale_factor=(1,2,2), mode='trilinear', align_corners=False)
        return self.conv(x)
...
class Block3D(nn.Module):
    def __init__(self, dim, dim_out, groups=8, activation='leakyrelu'):
        ...
        assert activation in ['leakyrelu', 'tanh']
        self.block = nn.Sequential(
            nn.Conv3d(dim, dim_out, kernel_size=(1,3,3), stride=1, padding=(0,1,1)),
...
class ResnetBlock(nn.Module):
    ...
    def forward(self, x, time_emb=None, c_emb=None):
        h = self.block1(x)

        if exists(time_emb):
            h += self.mlp(time_emb)[:, :, None, None]
            h += self.c_mlp(c_emb)[:, :, None, None]
...
class ResnetBlock3D(nn.Module):
    ...
    def forward(self, x, time_emb=None, c_emb=None):
        ...
        if exists(c_emb):
            h += self.c_mlp(c_emb)[:, :, None, None, None]
